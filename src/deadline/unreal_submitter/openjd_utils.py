...
from typing import Any, Dict, Type, get_type_hints, get_origin, get_args, Optional, Union
import inspect
import logging
            ...
            if args and len(args) > 0:
                ...
                # Handle Union types for list items
                    ...
                    for union_type in union_args:
                        if is_format_string_class(union_type) and field_name == "range" and "type" in result and result["type"] == "STRING":
                            # Special handling for TaskParameterStringValue in range fields
                            logger.debug(f"Converting list items to {union_type.__name__} for STRING parameter range")
                            if isinstance(value, list):
                                try:
                                    converted_items = []
                                    for item in value:
                                        if isinstance(item, str):
                                            logger.debug(f"Converting list item: {item} to {union_type.__name__}")
                                    ...
                                    logger.warning(f"Failed to convert list items to {union_type.__name__}: {e}")
...
def create_openjd_model(model_class, data_dict):
    ...
    try:
        ...
        for error in e.errors():
            ...
            if error.get('type') == 'value_error' and 'DynamicConstrainedStr' in error.get('msg', ''):
                ...
                if target_class:
                    ...
                    for i, path_part in enumerate(loc[:-1]):
                        ...
                        if isinstance(container, list):
                            ...
                            if isinstance(path_part, int) and 0 <= path_part < len(container):
                                ...
                                logger.warning(f"Invalid list index: {path_part} for {current_path}")
                                container = None
                                break
                            ...
                            logger.warning(f"Cannot navigate path: {current_path}, container is {type(container)}")
                            container = None
                            break
                    ...
                    if container is not None:
                        ...
                        if isinstance(container, list) and isinstance(last_part, int) and 0 <= last_part < len(container):
                            # Convert a list element
                            try:
                                container[last_part] = target_class(input_value)
                                logger.debug(f"Converted list item {last_part}: {input_value} to {target_class.__name__}")
                            except Exception as conv_error:
                                logger.warning(f"Failed to convert list item {last_part}: {conv_error}")
                        elif isinstance(container, dict) and last_part in container:
                            # Convert a dictionary value
                            try:
                                container[last_part] = target_class(input_value)
                                logger.debug(f"Converted dict value {last_part}: {input_value} to {target_class.__name__}")
                            except Exception as conv_error:
                                logger.warning(f"Failed to convert dict value {last_part}: {conv_error}")
                        else:
    ...
    try:
        ...
        logger.debug(f"Targeted conversions were not sufficient, falling back to full conversion: {e}")
